...
const logger = getLogger("appointmentCL");

export const handler = async (event: SQSEvent): Promise<SQSBatchResponse> => {
  logger.info("Processing CL appointments", { count: event.Records.length });

  const batchItemFailures: SQSBatchResponse["batchItemFailures"] = [];

  const connection = DatabaseFactory.getConnection("CL");
